...
test_that("bamboo.quantISORE (isoform quantification of bam file) produces expected output",{
  ...
  set.seed(1234)
  se = bamboo(reads =  Rsamtools::BamFileList(c(test.bam, test.bam), yieldSize = 1000),  annotations =  gr, genomeSequence = fa.file, algo.control = list(ncore = 1), extendAnnotations = FALSE)
  expect_type(se, "list")
  expect_equal(assays(se[["Transcript"]][,1]),assays(expectedSE[["Transcript"]]))
  expect_equal(assays(se[["Gene"]][,1]),assays(expectedSE[["Gene"]]))
  ...
  set.seed(1234)
  se = bamboo(reads =  Rsamtools::BamFileList(c(test.bam, test.bam), yieldSize = 1000), annotations =  gr, genomeSequence = fa.file, algo.control = list(ncore = 1), extendAnnotations = TRUE)
  expect_type(se, "list")
  expect_equal(assays(se[["Transcript"]][,1]),assays(expectedSE_extended[["Transcript"]]))
  expect_equal(assays(se[["Gene"]][,1]),assays(expectedSE_extended[["Gene"]]))
...
test_that("bamboo.preprocess (isoform quantification of bam file and save readClassFiles) produces expected output",{
  ...
  # test case 1: bamboo with single bam file, only using annotations (default option)
  set.seed(1234)
  se = bamboo(reads = test.bam,  annotations = txdb, genomeSequence = fa.file, algo.control = list(bias_correction = FALSE, ncore = 1), extendAnnotations = FALSE, outputReadClassDir = outputReadClassDir)
  expect_type(se, "list")
  expect_equal(assays(se[["Transcript"]][,1]),assays(expectedSE[["Transcript"]]))
  expect_equal(assays(se[["Gene"]][,1]),assays(expectedSE[["Gene"]]))
  ...
  set.seed(1234)
  se = bamboo(reads = test.bam,  annotations = gr, genomeSequence = fa.file, algo.control = list(bias_correction = FALSE, ncore = 1), extendAnnotations = FALSE, outputReadClassDir = outputReadClassDir)
  expect_type(se, "list")
  expect_equal(assays(se[["Transcript"]][,1]),assays(expectedSE[["Transcript"]]))
  expect_equal(assays(se[["Gene"]][,1]),assays(expectedSE[["Gene"]]))
  ...
  # test case 2: bamboo with multiple bam file, only using annotations (default option), yieldSize lower than read count
  set.seed(1234)
  se = bamboo(reads = Rsamtools::BamFileList(c(test.bam, test.bam), yieldSize = 1000),  annotations =  gr, genomeSequence = fa.file, algo.control = list(bias_correction = FALSE, ncore = 1), extendAnnotations = FALSE,outputReadClassDir = outputReadClassDir)
  expect_type(se, "list")
  expect_equal(assays(se[["Transcript"]][,1]),assays(expectedSE[["Transcript"]]))
  expect_equal(assays(se[["Gene"]][,1]),assays(expectedSE[["Gene"]]))
