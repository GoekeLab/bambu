...
testSpliceSites <- function(data, splice = "Start", # End
    ...
    if (any(mySet.all)) {
        mySet.training = which((annotatedSplice.prime | annotatedSplice)[mySet.all])
...
findJunctionsByStrand <- function(candidateJunctions,highConfidentJunctionSet,
    ...
    spliceSitePredictionList[is.na(spliceSitePredictionList)] <- 2 ## meaning NA
    ...
    for (maxDist in 0:10) {
        ...
        overlapByDist = 
            findOverlaps(candidateJunctions[which(is.na(mergedHighConfJunctionId))], 
                         candidateJunctions[which(candidateJunctions$highConfJunctionPrediction)],
