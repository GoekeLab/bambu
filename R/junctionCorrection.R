...
calculateStrandedReadCounts <- function(uniqueJunctions,
    ...
    junctionStrandList <- extractList(
    ...
    junctionSeqStart <- BSgenome::getSeq(
        genomeSequence,
        IRanges::shift(flank(uniqueJunctions,
            width = 2
        ), 2)
    ) # shift: from IRanges
    junctionSeqEnd <- BSgenome::getSeq(
        genomeSequence,
        IRanges::shift(flank(uniqueJunctions,
            width = 2, start = FALSE
        ), -2)
    ...
    junctionStartName <- paste(seqnames(uniqueJunctions),
        start(uniqueJunctions),
        sep = ":"
    )
...
createJunctionTable <- function(unlisted_junction_granges,
    ...
    if (is.null(genomeSequence)) {
        stop("Reference genome sequence is missing,
    ...
    if (is(genomeSequence, "character")) {
        if (grepl(".fa", genomeSequence)) {
            if (.Platform$OS.type == "windows") {
                ...
                newlevels <- unlist(lapply(strsplit(
            ...
            if (seqlevelsStyle(genomeSequence)[1] !=
                seqlevelsStyle(unlisted_junction_granges)[1]) {
                seqlevelsStyle(unlisted_junction_granges) <-
                    seqlevelsStyle(genomeSequence)[1]
            }
        } else {
            genomeSequence <- BSgenome::getBSgenome(genomeSequence)
            seqlevelsStyle(genomeSequence) <-
                seqlevelsStyle(unlisted_junction_granges)[1]
        }
    ...
    ## Todo: don't create junction names,
    ## instead work with indices/intergers (names are memory intensive)
    ...
    # calculate stranded read counts
    uniqueJunctions <- calculateStrandedReadCounts(
        uniqueJunctions,
        junctionMatchList,
        genomeSequence,
        unstranded_unlisted_junctions, unlisted_junction_granges
    )
    if (original_seqlevelstyle !=
        seqlevelsStyle(unlisted_junction_granges)[1]) {
    ...
    if (stranded == FALSE) {
        ...
        while (strandStep) {
            ...
            unlisted_junction_granges_strand <-
                as.character(strand(uniqueJunctionsUpdate)[subjectHits(allJunctionToUniqueJunctionOverlap)])
            ...
            updatedList <- updateJunctionwimprove(
                annotatedIntronNumber,
                uniqueJunctions, uniqueJunctionsUpdate, uniqueAnnotatedIntrons, strandStep, verbose
            )
        ...
        while (strandStep) {
            ...
            strand(uniqueJunctionsUpdate[strandScoreByRead < (0)]) <- "+"
            updatedList <- updateJunctionwimprove(annotatedIntronNumber, uniqueJunctions, uniqueJunctionsUpdate, uniqueAnnotatedIntrons, strandStep, verbose)
            annotatedIntronNumber <- updatedList$annotatedIntronNumber
            uniqueJunctions <- updatedList$uniqueJunctions
            strandStep <- updatedList$strandStep
        }
...
predictSpliceJunctions <- function(annotatedJunctions, junctionModel = NULL, verbose = FALSE) {
    ...
    testStartSplice <- testStartSpliceSites(annotatedJunctionsStart, annotatedJunctions, junctionModel)
    ...
    testEndSplice <- testEndSpliceSites(annotatedJunctionsEnd, annotatedJunctions, junctionModel)
