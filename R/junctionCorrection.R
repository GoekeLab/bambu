...
createJunctionTable <- function(unlisted_junction_granges,
    ...
    uniqueJunctions <- calculateStrandedReadCounts(uniqueJunctions,
        junctionMatchList,genomeSequence,
...
testStartSpliceSites <- function(annotatedJunctionsStart, annotatedJunctions,
    ...
    if (any(mySet.all)) {
        ...
        annotatedJunctions$spliceSitePredictionStart.start <-
            spliceSitePredictionStart.start[annotatedJunctions$junctionStartName]
    }
    mySet.all <- ((annotatedJunctionsStart$distStart.end != 0) &
        annotatedJunctionsStart$spliceStrand != "*" &
        annotatedJunctionsStart$startScore > 0 &
        abs(annotatedJunctionsStart$distStart.end) < 15)
    annotatedJunctions$spliceSitePredictionStart.end <-
        rep(NA, length(annotatedJunctions))
    if (any(mySet.all)) {
        mySet.training <- (annotatedJunctionsStart$annotatedStart.end |
            annotatedJunctionsStart$annotatedStart)[mySet.all]
        myData <- data.frame(
            annotatedJunctionsStart$startScore /
                (annotatedJunctionsStart$startScore.end + annotatedJunctionsStart$startScore),
            annotatedJunctionsStart$startScore, annotatedJunctionsStart$distStart.end,
            (annotatedJunctionsStart$spliceStrand.end == "+"),
            (annotatedJunctionsStart$spliceStrand.end == "-"),
            (annotatedJunctionsStart$spliceStrand == "+")
        )[mySet.all, ]
        colnames(myData) <- paste("A", seq_len(ncol(myData)), sep = ".")
        ...
        if (is.null(junctionModel)) {
            ...
            predictions <-
                glmnet:::predict.cv.glmnet(junctionModel[["spliceSitePredictionStart.end"]],
                    newx = modelmatrix, s = "lambda.min"
                )
        }
        ...
        annotatedJunctions$spliceSitePredictionStart.end <-
            spliceSitePredictionStart.end[annotatedJunctions$junctionStartName]
    }
    outputList <- list(
        "annotatedJunctions" = annotatedJunctions,
...
#' @param junctionModel junctionModel
#' @noRd
testEndSpliceSites <- function(annotatedJunctionsEnd, annotatedJunctions,
                               junctionModel) {
    mySet.all <- (annotatedJunctionsEnd$distEnd.start != 0 &
        annotatedJunctionsEnd$spliceStrand != "*" &
        annotatedJunctionsEnd$endScore > 0 &
        (annotatedJunctionsEnd$distEnd.start < 15))
    annotatedJunctions$spliceSitePredictionEnd.start <- rep(
        NA,
        length(annotatedJunctions)
    )
    if (any(mySet.all)) {
        mySet.training <- (annotatedJunctionsEnd$annotatedEnd.start |
            annotatedJunctionsEnd$annotatedEnd)[mySet.all]
        myData <- data.frame(
            annotatedJunctionsEnd$endScore /
                (annotatedJunctionsEnd$endScore.start + annotatedJunctionsEnd$endScore),
            annotatedJunctionsEnd$endScore, annotatedJunctionsEnd$distEnd.start,
            (annotatedJunctionsEnd$spliceStrand.start == "+"),
            annotatedJunctionsEnd$spliceStrand.start == "-",
            (annotatedJunctionsEnd$spliceStrand == "+")
        )[mySet.all, ]
        colnames(myData) <- paste("A", seq_len(ncol(myData)), sep = ".")
        ...
        if (is.null(junctionModel)) {
            myResults <- fitBinomialModel(
                labels.train =
                    as.integer(annotatedJunctionsEnd$annotatedEnd)[mySet.all][mySet.training],
                data.train = modelmatrix[mySet.training, ], data.test = modelmatrix,
                show.cv = verbose, maxSize.cv = 10000
            )
            ...
            predictions <-
                glmnet:::predict.cv.glmnet(junctionModel[["spliceSitePredictionEnd.start"]],
                    newx = modelmatrix, s = "lambda.min"
                )
        }
        ...
        annotatedJunctions$spliceSitePredictionEnd.start <-
            spliceSitePredictionEnd.start[annotatedJunctions$junctionEndName]
    }
    mySet.all <- (annotatedJunctionsEnd$distEnd.end != 0 &
        annotatedJunctionsEnd$spliceStrand != "*" &
        annotatedJunctionsEnd$endScore > 0 &
        abs(annotatedJunctionsEnd$distEnd.end) < 15)
    annotatedJunctions$spliceSitePredictionEnd.end <- rep(
        NA,
        length(annotatedJunctions)
    )
    if (any(mySet.all)) {
        mySet.training <- (annotatedJunctionsEnd$annotatedEnd.end |
            annotatedJunctionsEnd$annotatedEnd)[mySet.all]
        myData <- data.frame(
            annotatedJunctionsEnd$endScore /
                (annotatedJunctionsEnd$endScore.end + annotatedJunctionsEnd$endScore),
            annotatedJunctionsEnd$endScore, annotatedJunctionsEnd$distEnd.end,
            annotatedJunctionsEnd$distEnd.end, (annotatedJunctionsEnd$spliceStrand.end == "+"),
            (annotatedJunctionsEnd$spliceStrand.end == "-"),
            (annotatedJunctionsEnd$spliceStrand == "+")
        )[mySet.all, ]
        colnames(myData) <- paste("A", seq_len(ncol(myData)), sep = ".")
        modelmatrix <- model.matrix(~ A.1 + A.2 + A.3 + A.4 + A.5,
        ...
        if (is.null(junctionModel)) {
            ...
            predictions <-
                glmnet:::predict.cv.glmnet(junctionModel[["spliceSitePredictionEnd.end"]],
                    newx = modelmatrix, s = "lambda.min"
                )
        }
        ...
        annotatedJunctions$spliceSitePredictionEnd.end <-
            spliceSitePredictionEnd.end[annotatedJunctions$junctionEndName]
    }
    outputList <- list(
        "annotatedJunctions" = annotatedJunctions,
        "junctionModel" = junctionModel
...
predictSpliceJunctions <- function(annotatedJunctions, junctionModel = NULL,
    ...
    annotatedJunctionsStart <- unique(GRanges(
        seqnames =
            seqnames(annotatedJunctions), ranges = IRanges(
            start = start(annotatedJunctions),
            end = start(annotatedJunctions)
        ), strand = "*",
        mcols(annotatedJunctions)[, c(
            "startScore", "junctionStartName",
            "annotatedStart", "spliceStrand", "spliceMotif"
        )]
    ))
    annotatedJunctionsEnd <- sort(unique(GRanges(
        seqnames =
            seqnames(annotatedJunctions), ranges = IRanges(
            start = end(annotatedJunctions),
            end = end(annotatedJunctions)
        ), strand = "*", mcols(annotatedJunctions)[
            ,
            c(
                "endScore", "junctionEndName", "annotatedEnd", "spliceStrand",
                "spliceMotif"
            )
        ]
    )))
    ...
    annotatedJunctionsStart$distStart.start <- c(
        0,
        (start(annotatedJunctionsStart[-1]) -
            start(annotatedJunctionsStart[-length(annotatedJunctionsStart)])) *
            as.integer((seqnames(annotatedJunctionsStart[-1]) ==
                seqnames(annotatedJunctionsStart[-length(annotatedJunctionsStart)])))
    )
    annotatedJunctionsStart$annotatedStart.start <- c(
        FALSE,
        annotatedJunctionsStart$annotatedStart[-length(annotatedJunctionsStart)]
    )
    annotatedJunctionsStart$startScore.start <- c(
        FALSE,
        annotatedJunctionsStart$startScore[-length(annotatedJunctionsStart)]
    )
    annotatedJunctionsStart$spliceStrand.start <- c(
        FALSE,
        annotatedJunctionsStart$spliceStrand[-length(annotatedJunctionsStart)]
    )
    annotatedJunctionsStart$spliceMotif.start <- c(
        FALSE,
        annotatedJunctionsStart$spliceMotif[-length(annotatedJunctionsStart)]
    )
    ## test start splice site given close by splice site (right/3')
    annotatedJunctionsStart$distStart.end <-
        c((end(annotatedJunctionsStart[-length(annotatedJunctionsStart)]) -
            end(annotatedJunctionsStart[-1])) *
            as.integer((seqnames(annotatedJunctionsStart[-length(annotatedJunctionsStart)]) ==
                seqnames(annotatedJunctionsStart[-1]))), 0)
    annotatedJunctionsStart$annotatedStart.end <-
        c(annotatedJunctionsStart$annotatedStart[-1], FALSE)
    annotatedJunctionsStart$startScore.end <-
        c(annotatedJunctionsStart$startScore[-1], FALSE)
    annotatedJunctionsStart$spliceStrand.end <-
        c(annotatedJunctionsStart$spliceStrand[-1], FALSE)
    annotatedJunctionsStart$spliceMotif.end <-
        c(annotatedJunctionsStart$spliceMotif[-1], FALSE)
    testStartSplice <-
        testStartSpliceSites(annotatedJunctionsStart, annotatedJunctions, junctionModel)
    annotatedJunctions <- testStartSplice$annotatedJunctions
    junctionModel <- testStartSplice$junctionModel
    ## test end splice site given close by splice site (start/5')
    annotatedJunctionsEnd$distEnd.start <- c(0, (start(annotatedJunctionsEnd[-1]) -
        start(annotatedJunctionsEnd[-length(annotatedJunctionsEnd)])) *
        as.integer((seqnames(annotatedJunctionsEnd[-1]) ==
            seqnames(annotatedJunctionsEnd[-length(annotatedJunctionsEnd)]))))
    annotatedJunctionsEnd$annotatedEnd.start <- c(
        FALSE,
        annotatedJunctionsEnd$annotatedEnd[-length(annotatedJunctionsEnd)]
    )
    annotatedJunctionsEnd$endScore.start <- c(
        FALSE,
        annotatedJunctionsEnd$endScore[-length(annotatedJunctionsEnd)]
    )
    annotatedJunctionsEnd$spliceStrand.start <- c(
        FALSE,
        annotatedJunctionsEnd$spliceStrand[-length(annotatedJunctionsEnd)]
    )
    annotatedJunctionsEnd$spliceMotif.start <- c(
        FALSE,
        annotatedJunctionsEnd$spliceMotif[-length(annotatedJunctionsEnd)]
    )
    ## test end splice site given close by splice site (right/3')
    annotatedJunctionsEnd$distEnd.end <-
        c((end(annotatedJunctionsEnd[-length(annotatedJunctionsEnd)]) -
            end(annotatedJunctionsEnd[-1])) *
            as.integer((seqnames(annotatedJunctionsEnd[-length(annotatedJunctionsEnd)]) ==
                seqnames(annotatedJunctionsEnd[-1]))), 0)
    annotatedJunctionsEnd$annotatedEnd.end <- c(
        annotatedJunctionsEnd$annotatedEnd[-1],
        FALSE
    )
    annotatedJunctionsEnd$endScore.end <- c(
        annotatedJunctionsEnd$endScore[-1],
        FALSE
    )
    annotatedJunctionsEnd$spliceStrand.end <- c(
        annotatedJunctionsEnd$spliceStrand[-1],
        FALSE
    )
    annotatedJunctionsEnd$spliceMotif.end <- c(
        annotatedJunctionsEnd$spliceMotif[-1],
        FALSE
    )
    testEndSplice <- testEndSpliceSites(
        annotatedJunctionsEnd, annotatedJunctions,
        junctionModel
    )
    annotatedJunctions <- testEndSplice$annotatedJunctions
    junctionModel <- testEndSplice$junctionModel
    if (is.null(junctionModel)) {
...
fitBinomialModel <- function(labels.train, data.train, data.test,
    ...
    if (show.cv) {
        ...
        predictions <- glmnet:::predict.cv.glmnet(cv.fit,
            newx = data.train.cv.test, s = "lambda.min"
        )
        message("prediction accuracy (CV) (higher for splice donor than
          splice acceptor)")
...
useRefJunctionForConflict <- function(junctions, candidateJunctionsMinus,
    ...
    conflictJunctions <-
        junctions[names(candidateJunctionsMinus[!is.na(
    ...
    scoreDiff <-
        junctions[candidateJunctionsPlus[names(
            conflictJunctions
        )]$mergedHighConfJunctionId]$score -
        junctions[candidateJunctionsMinus[names(
            conflictJunctions
        )]$mergedHighConfJunctionId]$score
    if (any(scoreDiff < 0)) {
        candidateJunctionsPlus[names(
            conflictJunctions
        )]$mergedHighConfJunctionId[scoreDiff < 0] <-
            candidateJunctionsMinus[names(
                conflictJunctions
            )]$mergedHighConfJunctionId[scoreDiff < 0]
    }
    if (any(scoreDiff > 0)) {
    ...
    if (0 %in% scoreDiff) {
        candidateJunctionsPlus[names(
            conflictJunctions
        )]$mergedHighConfJunctionId[scoreDiff == 0] <- NA
        candidateJunctionsMinus[names(
            conflictJunctions
        )]$mergedHighConfJunctionId[scoreDiff == 0] <- NA
    }
...
#'  this function adds "mergedHighConfJunctionId" to the junciton list which
#'  contains the ID of the most likely high confident junction that each junction originates from
#' @noRd
findHighConfidenceJunctions <- function(junctions, junctionModel, verbose = FALSE) {
    ...
    if (sum(highConfidentJunctionSetPlus) > 0 & sum(highConfidentJunctionSetMinus) > 0) {
        ...
        junctions <- useRefJunctionForConflict(junctions, candidateJunctionsMinus, candidateJunctionsPlus)
    } else {
