...
checkEdgeExonsWithinIntronBoundaries = function(query, subject, threshold = 35){
    ...
    unlistedIntrons <- unlist(introns, use.names = FALSE)
    partitioning <- PartitioningByEnd(cumsum(sum(width(introns)>threshold)), names = NULL)
    unlistedIntrons = unlistedIntrons[width(unlistedIntrons)>threshold]
    start(unlistedIntrons) <- pmin(start(unlistedIntrons)+threshold, end(unlistedIntrons))
    end(unlistedIntrons) <- pmax(end(unlistedIntrons)-threshold, start(unlistedIntrons))
    introns = relist(unlistedIntrons, partitioning)
