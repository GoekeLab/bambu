...
prepareAnnotations <- function(x) {
  if(is(x,"TxDb")){
    ...
    unlistedExons$exon_endRank <- unlist(vapply(elementNROWS(exonsByTx),seq,FUN.VALUE = numeric(1), Fto=1), use.names=FALSE)
...
prepareAnnotationsFromGTF <- function(file){
  if (missing(file)){
    ...
    exon_rank <- vapply(elementNROWS(grlist), seq, FUN.VALUE = numeric(1), from = 1)
    ...
    unlistedExons <- unlistedExons[order(txIdForReorder, unlistedExons$exon_rank)]  #'exonsByTx' is always sorted by exon rank, not by strand, make sure that this is the case here
    unlistedExons$exon_endRank <- unlist(vapply(elementNROWS(grlist),seq,FUN.VALUE = numeric(1),to=1), use.names=FALSE)
    unlistedExons <- unlistedExons[order(txIdForReorder, start(unlistedExons))]
