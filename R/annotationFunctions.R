...
prepareAnnotations <- function(x) {
  if(is(x,"TxDb")){
    ...
    unlistedExons$exon_endRank <- unlist(vapply(elementNROWS(exonsByTx),seq,FUN.VALUE = numeric(length(exonsByTx)), Fto=1), use.names=FALSE)
...
prepareAnnotationsFromGTF <- function(file){
  if (missing(file)){
    ...
    exon_rank <- vapply(elementNROWS(grlist), seq, FUN.VALUE = numeric(length(grlist)), from = 1)
    ...
    unlistedExons <- unlistedExons[order(txIdForReorder, unlistedExons$exon_rank)]  #'exonsByTx' is always sorted by exon rank, not by strand, make sure that this is the case here
    unlistedExons$exon_endRank <- unlist(vapply(elementNROWS(grlist),seq,FUN.VALUE = numeric(length(grlist)),to=1), use.names=FALSE)
    unlistedExons <- unlistedExons[order(txIdForReorder, start(unlistedExons))]
...
txRangesToGeneRanges <- function(exByTx, TXNAMEGENEID_Map){
  ...
  exon_rank <- vapply(width((partitioning)), seq,FUN.VALUE = numeric(partitioning), from=1)
