...
createReadTable <- function(uniqueJunctions,unlisted_junctions,readGrgList,
  ...
  readTable[, 'intronStarts'] <- 
    unstrsplit(splitAsList(as.character(intronStartTMP), 
                           mcols(unlisted_junctions)$id), sep=',') 
  readTable[, 'intronEnds'] <- 
    unstrsplit(splitAsList(as.character(intronEndTMP), 
                           mcols(unlisted_junctions)$id), sep=',') 
  #start and end
  intronStartCoordinatesInt <- 
    as.integer(min(splitAsList(intronStartTMP, 
                               mcols(unlisted_junctions)$id))-2)
  intronEndCoordinatesInt <- 
    as.integer(max(splitAsList(intronEndTMP, 
                               mcols(unlisted_junctions)$id))+2)
  ...
  readTable <- readTable %>% group_by(chr, strand, intronEnds, 
                                      intronStarts) %>%
               summarise(readCount=n(), start=nth(x=start, 
...
constructSplicedReadClassTables <- function(uniqueJunctions, 
  ...
  allJunctionToUniqueJunctionOverlap <- 
    findOverlaps(unlisted_junctions,uniqueJunctions,type = 'equal',
                 ignore.strand = TRUE)
  intronStartTMP <- 
  ...
  readTable <- readTable %>% dplyr::select(chr.rc = chr, 
