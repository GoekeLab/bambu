...
createReadTable <- function(uniqueJunctions,unlisted_junctions,readGrgList,
  ...
  readTable <- readTable %>% group_by(chr, strand, intronEnds, intronStarts) %>%
               summarise(readCount=n(), start=nth(x=start, n=ceiling(readCount/5), order_by = start),
...
constructSplicedReadClassTables <- function(uniqueJunctions, unlisted_junctions, readGrgList, stranded=FALSE){
  ...
  allJunctionToUniqueJunctionOverlap <- findOverlaps(unlisted_junctions,uniqueJunctions,type = 'equal',ignore.strand = TRUE)
  ...
  readTable <- createReadTable(uniqueJunctions,unlisted_junctions,readGrgList,
                               firstseg,intronStartTMP,intronEndTMP,readStrand,
                               allJunctionToUniqueJunctionOverlap)
  ...
  exonsByReadClass <- makeGRangesListFromFeatureFragments(seqnames = readTable$chr,fragmentStarts = exonStartsShifted,fragmentEnds = exonEndsShifted,strand = readTable$strand)
  ...
  exon_rank <- lapply(width((partitioning)), seq, from = 1) #add exon rank and exon_endRank
  ...
  exonsByReadClass <- relist(unlistData, partitioning) ## combine new transcripts with annotated transcripts based on identical intron pattern
  readTable <- readTable %>% dplyr::select(chr.rc = chr, strand.rc = strand, intronStarts, intronEnds, confidenceType, readCount)
