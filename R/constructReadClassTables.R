...
createReadTable <- function(uniqueJunctions, unlisted_junctions, readGrgList,
                            ...
                            allJunctionToUniqueJunctionOverlap) {
    readTable <- as_tibble(data.frame(matrix(
        ncol = 7,
        nrow = length(readGrgList)
    )))
    colnames(readTable) <- c(
        "chr", "start", "end", "strand", "intronEnds",
    ...
    readTable[, "intronStarts"] <-
        unstrsplit(splitAsList(
            as.character(intronStartTMP),
            mcols(unlisted_junctions)$id
        ), sep = ",")
    readTable[, "intronEnds"] <-
        unstrsplit(splitAsList(
            as.character(intronEndTMP),
            mcols(unlisted_junctions)$id
        ), sep = ",")
    # start and end
    intronStartCoordinatesInt <-
        as.integer(min(splitAsList(
            intronStartTMP,
            mcols(unlisted_junctions)$id
        )) - 2)
    intronEndCoordinatesInt <-
        as.integer(max(splitAsList(
            intronEndTMP,
            mcols(unlisted_junctions)$id
        )) + 2)
    ...
    readTable <- readTable %>%
        group_by(
            chr, strand, intronEnds,
            intronStarts
        ) %>%
        summarise(
            readCount = n(), start = nth(
                x = start,
                n = ceiling(readCount / 5), order_by = start
            ),
...
constructSplicedReadClassTables <- function(uniqueJunctions,
    ...
    exon_0size <- which(intronStartTMP[-1] <=
        intronEndTMP[-length(intronEndTMP)] &
    ...
    if (!stranded) {
        ...
        plusCount <- as.integer(sum(splitAsList(
            unlisted_junctions_strand,
            mcols(unlisted_junctions)$id
        ) == "+"))
        minusCount <- as.integer(sum(splitAsList(
            unlisted_junctions_strand,
            mcols(unlisted_junctions)$id
        ) == "-"))
    ...
    exonEndsShifted <- paste(readTable$intronStarts,
        readTable$end + 1,
        sep = ","
    )
    exonStartsShifted <- paste(readTable$start - 1,
        readTable$intronEnds,
        sep = ","
    )
    exonsByReadClass <-
        makeGRangesListFromFeatureFragments(
            ...
            fragmentStarts = exonStartsShifted, fragmentEnds = exonEndsShifted,
    ...
    readTable <- readTable %>% dplyr::select(
        chr.rc = chr,
        strand.rc = strand, intronStarts, intronEnds,
...
constructUnsplicedReadClasses <- function(granges,
    ...
    hitsDF <- hitsDF %>%
        ...
        ungroup() %>%
        mutate(
            confidenceType = confidenceType, intronStarts = NA,
            intronEnds = NA
        ) %>%
        dplyr::select(
