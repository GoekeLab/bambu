...
bambu <- function(reads = NULL, readClass.file = NULL, readClass.outputDir = NULL,
    ...
    readClassList <- processReads(
        reads, annotations, genomeSequence,
        readClass.outputDir,
...
bambu.quantify <- function(readClass, annotations, emParameters,
    ...
    seOutput <- SummarizedExperiment::SummarizedExperiment(
        assays =
            SimpleList(
                counts = matrix(counts$estimates, ncol = 1, dimnames = list(NULL, colNameRC)),
...
bambu.quantDT <- function(readClassDt = readClassDt, emParameters = NULL,
    ...
    theta_est <- outList[[1]]
    theta_est[, `:=`(
        tx_name = txVec[as.numeric(tx_sid)],
