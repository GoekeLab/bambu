...
isore.extendAnnotations <- function(combinedTranscripts, annotationGrangesList,
    ...
    combinedTranscripts <- 
        filterTranscripts(combinedTranscripts,min.sampleNumber)
    ...
    rowDataFilteredSpliced <- addNewSplicedReadClasses(transcriptRanges,
        rowDataSplicedTibble, annotationGrangesList, min.exonDistance, 
        min.primarySecondaryDist,min.primarySecondaryDistStartEnd, verbose)
    rowDataFilteredUnspliced <- 
        rowDataTibble[which(confidenceTypeVec == "unsplicedNew"),]
...
filterTranscriptsByAnnotation <- function(rowDataCombined,annotationGrangesList,
    ...
    if (remove.subsetTx) { # (1) based on compatiblity with annotations
        ...
        rowDataCombined$readClassType == "equalcompatible") #keep equal for NDR
    ...
    filterSet = (rowDataCombined$txNDR <= max.txNDR) & 
        !rowDataCombined$readClassType == "equalcompatible"
    ...
    extendedAnnotationRanges <- removeTranscriptsWIdenJunct(
        rowDataCombined, exonRangesCombined, annotationGrangesList, prefix)
    end.ptm <- proc.time()
...
calculateNDROnTranscripts <- function(combinedTranscripts){
    ...
    if(sum(equal, na.rm = TRUE)<50 | 
        ...
        warning("Less than 50 TRUE or FALSE read classes for precision 
        stabilization. Filtering by prediction score instead")
    } else combinedTranscripts$txNDR = 
        calculateNDR(combinedTranscripts$maxTxScore, equal)
    return(combinedTranscripts)
}
...
calculateNDR = function(score, labels){
    ...
    NDR = rev(cummin(rev(NDR))) #flatten NDR to maximize sensitivty at anNDR
...
addNewSplicedReadClasses <- function(combinedTranscriptRanges, 
    ...
    # annotate with transcript, gene Ids and compatible gene id,
    ...
    classificationTable <- updateWIntronMatches(unlistedIntrons, 
        unlistedIntronsAnnotations, partitioning, classificationTable, 
        annotationGrangesList, rowDataFilteredSpliced, exonsByReadClass, 
    ...
    if (verbose) message("extended annotations for spliced reads in ",
        round((proc.time() - start.ptm)[3] / 60, 1), " mins.")
...
updateWIntronMatches <- function(unlistedIntrons, unlistedIntronsAnnotations,
    ...
    classificationTable$newLastJunction[which( strandVec == "+" & 
        !lastJunctionMatch & intronMatchAny | (strandVec == "-" & 
        (!firstJunctionMatch & intronMatchAny)))] <- "newLastJunction"
    classificationTable$newFirstJunction[which(strandVec == "+" & 
        !firstJunctionMatch & intronMatchAny | (strandVec == "-" & 
        (!lastJunctionMatch & intronMatchAny)))] <- "newFirstJunction"
...
#' @noRd
addNewUnsplicedReadClasses <- function(rowDataFilteredUnspliced,  
        rowDataFilteredSpliced, exonsByReadClass, annotationGrangesList, 
...
assignGeneIDexonMatch <- function(rowDataCombined, exonRangesCombined,
    ...
    if (any(isNaGeneIdAfter)) {
        newGeneIds <- assignGeneIdsNoReference(
            exonRangesCombined[isNaGeneIdAfter],
        prefix = prefix)
