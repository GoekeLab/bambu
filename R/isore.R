...
createModelforJunctionReads <- function(readGrgList, annotationGrangesList, unlisted_junctions,
  ...
  uniqueJunctions <- junctionTables[[1]][,c('score', 'spliceMotif', 'spliceStrand','junctionStartName','junctionEndName', 'startScore','endScore', 'id')]
  ...
  # build model to predict true splice sites and correct junctions based on set of high confidence junctions
...
#' @inheritParams bambu
#'@noRd
isore.constructReadClasses <- function(readGrgList, runName='sample1', annotationGrangesList,
                                       genomeSequence=NULL, stranded=FALSE, ncore = 1, verbose=FALSE){
  ## todo: which preprocessed junction correction model to use? #standardJunctionModels_temp
  unlisted_junctions <- unlistIntrons(readGrgList, use.ids=TRUE, use.names=FALSE)
  start.ptm <- proc.time() # cat('### create junction list with splice motif ### \n')
  ...
  if(!all(seqlevels(unlisted_junctions) %in% seqlevels(uniqueJunctions))) { # warning is already shown when ranges are dropped the first time
  ...
  readClassListSpliced <- createModelforJunctionReads(readGrgList, annotationGrangesList, unlisted_junctions,
                                                      uniqueJunctions, stranded, verbose)
  #  cat('### create single exon transcript models (read classes) ### \n')
  start.ptm <- proc.time() # seqlevels are made equal (added for chromosomes missing in any of them)
  ...
  se <- SummarizedExperiment(assays = SimpleList(counts = counts), rowRanges = exonsByReadClass, colData = colDataDf)
...
createSEforSplicedTx <- function(rowData.spliced, readClassSeRef, readClassSe, colDataCombined){
  ...
  start.splicedRef <- matrix(NA,dimnames = list(seq_len(nrow(rowData.spliced)),
                                                rownames(colData(readClassSeRef))),
                             ncol = nrow(colData(readClassSeRef)),
                             nrow = nrow(rowData.spliced))
...
prepSEforUnsplicedTx <- function(unsplicedRanges, combinedSingleExonRanges, readClass, stranded, readClassSeTBL = NULL){
  overlapToCombined <-findOverlaps(unsplicedRanges,
                                   ...
                                   select='first')
  counts.unsplicedSum <- as_tibble(assays(readClass)[['counts']])[rowData(readClass)$confidenceType=='unsplicedNew',] %>%
                         mutate(index=overlapToCombined) %>%
                         group_by(index) %>%
                         summarise_all(sum, na.rm=TRUE)
  if (is.null(readClassSeTBL)){
    start.unsplicedSum <- as_tibble(assays(readClass)[['start']])[rowData(readClass)$confidenceType=='unsplicedNew',] %>%
      mutate(index=overlapToCombined) %>%
      group_by(index) %>%
      summarise_all(min, na.rm=TRUE)
    end.unsplicedSum <- as_tibble(assays(readClass)[['end']])[rowData(readClass)$confidenceType=='unsplicedNew',] %>%
      mutate(index=overlapToCombined) %>%
      group_by(index) %>%
  ...
  tableList <- list("counts.unsplicedSum"= counts.unsplicedSum, 
...
createSEforUnsplicedTx <- function(readClassSeRef, readClassSe, readClassSeTBL,
  ...
  refTables <- prepSEforUnsplicedTx(unsplicedRangesRef, combinedSingleExonRanges, readClassSeRef, stranded)
  ...
  newTables <- prepSEforUnsplicedTx(unsplicedRangesNew, combinedSingleExonRanges, readClassSe, stranded, readClassSeTBL)
  ...
  counts.unsplicedRef <- matrix(0, dimnames = list(seq_len(nrow(rowData.unspliced)),
                                                  rownames(colData(readClassSeRef))),
                                   ncol = nrow(colData(readClassSeRef)),
                                   nrow = nrow(rowData.unspliced))
  start.unsplicedRef <- matrix(NA,dimnames = list(seq_len(nrow(rowData.unspliced)),
                                                  rownames(colData(readClassSeRef))),
                                  ncol = nrow(colData(readClassSeRef)),
                                  nrow = nrow(rowData.unspliced))
  end.unsplicedRef <- start.unsplicedRef
  
  counts.unsplicedNew <- matrix(0, dimnames = list(seq_len(nrow(rowData.unspliced)),
                                                   rownames(colData(readClassSe))),
                                   ncol = nrow(colData(readClassSe)),
                                   nrow = nrow(rowData.unspliced))
  start.unsplicedNew <- matrix(NA, dimnames = list(seq_len(nrow(rowData.unspliced)),
                                                   rownames(colData(readClassSe))),
                                   ncol = nrow(colData(readClassSe)),
                                   nrow = nrow(rowData.unspliced))
  ...
  counts.unsplicedNew[counts.unsplicedNewSum$index, ] <- as.matrix(counts.unsplicedNewSum[,colnames(counts.unsplicedNew)])
  start.unsplicedNew[counts.unsplicedNewSum$index, ] <- as.matrix(start.unsplicedNewSum[,'start'])
  end.unsplicedNew[counts.unsplicedNewSum$index, ] <- as.matrix(end.unsplicedNewSum[,'end'])
  ############ TODO: Speed up ###############
...
isore.combineTranscriptCandidates <- function(readClassSe, readClassSeRef = NULL, stranded = FALSE, verbose = FALSE){
  if(is.null(readClassSeRef)){  #if no reference object is given, create one from a readClassSe object
    ...
    readClassSeTBL <- as_tibble(rowData(readClassSe), rownames='id') %>%
                      mutate(start=min(start(rowRanges(readClassSe))), end=max(end(rowRanges(readClassSe))))
    ...
    se.spliced <- createSEforSplicedTx(rowData.spliced, readClassSeRef, readClassSe, colDataCombined)
    ...
    se.unspliced <- createSEforUnsplicedTx(readClassSeRef, readClassSe, readClassSeTBL,
                                           unsplicedRangesRef, unsplicedRangesNew, 
                                           combinedSingleExonRanges, colDataCombined, 
                                           rowData.unspliced, stranded)
                                    ...
                                    verbose=FALSE)  ## prefix for new gene Ids (genePrefix.number)
{
  ...
  if(sum(filterSet1) > 0){
    if(any(rowData(se)$confidenceType == 'highConfidenceJunctionReads' & filterSet1)){
      ...
      colnames(classificationTable) <- c('equal',
                                         'compatible',
                                         'newWithin',
                                         'newLastJunction',
                                         'newFirstJunction',
      ...
      distNewTx <- calculateDistToAnnotation(exByTx=exonsByReadClass,
      ...
      distNewTxByQuery <- distNewTx %>%
        ...
        summarise(minDist=min(dist),
                  ...
                  compatible=any(compatible))  ## note: here is more information that can be used to filter and annotate!
...
extdannotateSplicedReads <- function(exonsByReadClass, intronsByReadClass, annotationGrangesList, 
  ...
  colnames(classificationTable) <- c('equal', 'compatible', 'newWithin', 'newLastJunction', 'newFirstJunction',
                                     'newJunction', 'allNew', 'newFirstExon', 'newLastExon')
  classificationTable$equal[queryHits(ovExon[mcols(ovExon)$equal])[!duplicated(queryHits(ovExon[mcols(ovExon)$equal]))]] <- 'equal' # annotate as identical
  classificationTable$compatible[queryHits(ovExon[mcols(ovExon)$compatible ])[!duplicated(queryHits(ovExon[mcols(ovExon)$compatible]))]] <- 'compatible' ##compatible, ( same last exon/ different last exon can be separated later)
  classificationTable$compatible[classificationTable$equal=='equal'] <- ''
  ## annotate with transcript and gene Ids
  ...
  ## new within annotations (all junctions known)
  classificationTable$newWithin[all(intronMatchesList) & ! (classificationTable$compatible=='compatible' | classificationTable$equal == 'equal')] <- 'newWithin'
  ## new with new junction internal (new splice variant), new with new junction first (new TSS/first exons), and new with new junction last (new last exon)
  lastJunctionMatch <- unlist(endoapply(endoapply(intronMatchesList, rev), '[[', 1))
  firstJunctionMatch <- unlist(endoapply(intronMatchesList, '[[', 1))
  classificationTable$newLastJunction[which(rowData(seFilteredSpliced)$strand == '+' & !lastJunctionMatch & any(intronMatchesList))] <- 'newLastJunction'
  classificationTable$newLastJunction[which(rowData(seFilteredSpliced)$strand == '-' & (!firstJunctionMatch & any(intronMatchesList)))] <- 'newLastJunction'
  ...
  if(length(overlapsNewIntronsAnnotatedIntrons)>0){
    distNewTxByQuery <- assignGeneIDbyMaxMatch(unlistedIntrons, unlistedIntronsAnnotations, 
  ...
  if(verbose)  message('extended annotations for spliced reads in ', round((end.ptm-start.ptm)[3]/60,1), ' mins.')
...
filterTranscripts <- function(seCombined, annotationGrangesList, exonRangesCombined, prefix,
  ...
  mcols(seCombinedFiltered)$newTxClass[mcols(seCombinedFiltered)$readClassType == 'unsplicedNew' &
                                         grepl('gene', mcols(seCombinedFiltered)$GENEID)] <- 'newGene-unspliced'
  mcols(seCombinedFiltered)$newTxClass[mcols(seCombinedFiltered)$readClassType=='allNew' &
                                         grepl('gene', mcols(seCombinedFiltered)$GENEID)] <- 'newGene-spliced'
...
if(sum(filterSet1) > 0){
  if(any(rowData(se)$confidenceType == 'highConfidenceJunctionReads' & filterSet1)){
    ...
    names(intronsByReadClass) <- seq_along(intronsByReadClass)
    seqlevels(intronsByReadClass) <-  unique(c(seqlevels(intronsByReadClass), seqlevels(annotationGrangesList)))
    exonsByReadClass <- createExonByReadClass(seFilteredSpliced, annotationGrangesList)
    seFilteredSpliced <- extdannotateSplicedReads(exonsByReadClass, intronsByReadClass, annotationGrangesList, seFilteredSpliced, min.exonDistance, verbose)
  }
  ## unspliced transcripts
  SEnRng <- extdannotateUnsplicedReads(se, seFilteredSpliced, exonsByReadClass, 
  ...
  seCombined <- assignGeneIDexonMatch(seCombined, exonRangesCombined, annotationGrangesList, 
                                      min.exonOverlap, prefix, verbose)
  ## filter out transcripts
  extendedAnnotationRanges <- filterTranscripts(seCombined, annotationGrangesList, exonRangesCombined,
                                                prefix, min.readFractionByGene, min.sampleNumber, 
...
isore.estimateDistanceToAnnotations <- function(seReadClass, annotationGrangesList, min.exonDistance = 35, additionalFiltering = FALSE, verbose = FALSE){
  ...
  distTable <- left_join(distTable,as_tibble(mcols(annotationGrangesList)[,c('TXNAME','GENEID')]),
