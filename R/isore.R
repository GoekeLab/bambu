...
createModelforJunctionReads <- function(readGrgList, annotationGrangesList, 
  ...
  uniqueJunctions$annotatedJunction <- (!is.na(GenomicRanges::match(uniqueJunctions, 
  ...
  if(verbose)  message('Finished create transcript models 
...
correctJunctionFromPrediction <- function(uniqueJunctions,verbose){
  ...
  if(sum(uniqueJunctions$annotatedJunction)>5000 & 
    ...
    predictSpliceSites <- predictSpliceJunctions(
                                                 ...
                                                 junctionModel = NULL,
                                                 verbose = verbose)
    uniqueJunctions=predictSpliceSites[[1]][,c('score', 'spliceMotif',
                                               'spliceStrand','junctionStartName','junctionEndName',
  ...
  if(verbose) message('Model to predict true splice sites built in ', 
                      round((end.ptm-start.ptm)[3]/60,1), ' mins.')
  ...
  if(verbose) message('Finished correcting junction 
...
isore.constructReadClasses <- function(readGrgList, 
  ...
  readClassListSpliced <- createModelforJunctionReads(readGrgList, 
  ...
  referenceExons <- unique(c(granges(unlist(
    ...
    use.names=FALSE)), granges(unlist(annotationGrangesList, use.names=FALSE))))
...
createSEforSplicedTx <- function(rowData.spliced, readClassSeRef, 
  ...
  se.spliced <- SummarizedExperiment(assays = SimpleList(
...
createSEforUnsplicedTx <- function(readClassSeRef, readClassSe, 
  ...
  ## is slow, replace with more efficient method?
  se.unspliced <- SummarizedExperiment(assays = SimpleList(
...
isore.combineTranscriptCandidates <- function(readClassSe, 
  ...
  if(is.null(readClassSeRef)){  
    ...
    rowData <- rowData %>% dplyr::select(chr=chr.rc, start, 
    ...
    readClassSeRefTBL.unspliced <- filter(readClassSeRefTBL, 
                                          confidenceType=='unsplicedNew')
    readClassSeTBL.unspliced <- filter(readClassSeTBL, 
                                       confidenceType=='unsplicedNew')
    ...
    combinedSingleExonRanges <- reduce(c(unsplicedRangesRef,
                                         unsplicedRangesNew), ignore.strand=!stranded)
    ...
    se.unspliced <- createSEforUnsplicedTx(readClassSeRef, 
...
extdannotateSplicedReads <- function(exonsByReadClass, 
  ...
  colnames(classificationTable) <- c('equal', 'compatible', 
  ...
  lastJunctionMatch <- unlist(endoapply(endoapply(intronMatchesList, 
                                                  rev), '[[', 1))
  firstJunctionMatch <- unlist(endoapply(intronMatchesList, '[[', 1))
  classificationTable$newLastJunction[which(rowData(
    seFilteredSpliced)$strand == '+' & !lastJunctionMatch & any(
      intronMatchesList))] <- 'newLastJunction'
  classificationTable$newLastJunction[which(rowData(
    seFilteredSpliced)$strand == '-' & (!firstJunctionMatch & any(
      intronMatchesList)))] <- 'newLastJunction'
  classificationTable$newFirstJunction[which(rowData(
    seFilteredSpliced)$strand == '+' & !firstJunctionMatch & any(
      intronMatchesList))] <- 'newFirstJunction'
  classificationTable$newFirstJunction[which(rowData(
    seFilteredSpliced)$strand == '-' & (!lastJunctionMatch & any(
      intronMatchesList)))] <- 'newFirstJunction'
  ...
  if(length(overlapsNewIntronsAnnotatedIntrons)>0){
    distNewTxByQuery <- assignGeneIDbyMaxMatch(unlistedIntrons, 
    ...
    classificationTable$newLastExon[
      distNewTxByQuery$queryHits[!distNewTxByQuery$endMatch]] <- 'newLastExon'
    classificationTable$newLastExon[
...
assignGeneIDbyMaxMatch <- function(unlistedIntrons, 
  ...
  mcols(seFilteredSpliced)$GENEID[is.na(
    mcols(seFilteredSpliced)$GENEID)] <- geneIdByIntron[is.na(
...
extdannotateUnsplicedReads <- function(se, seFilteredSpliced, 
  ...
  if(any(rowData(se)$confidenceType == 'unsplicedNew' & filterSet1)) {
    ...
    seqlevels(exonsByReadClassUnspliced) <-  unique(c(seqlevels(exonsByReadClassUnspliced),
                                                      seqlevels(annotationGrangesList)))
    ...
    seCombined <- SummarizedExperiment::rbind(seFilteredSpliced, 
                                              seFilteredUnspliced)
  ...
  if(verbose)  message('extended annotations for unspliced reads in ', 
                       round((end.ptm-start.ptm)[3]/60,1), ' mins.')
  outputList <- list("seCombined" = seCombined, 
...
assignGeneIDexonMatch <- function(seCombined, exonRangesCombined, 
  ...
  geneIdByExon[!is.na(exonMatchGene)] <- mcols(
    annotationGrangesList)$GENEID[exonMatchGene[!is.na(exonMatchGene)]]
  geneIdByExon[!is.na(mcols(seCombined)$GENEID)] <-  mcols(
    seCombined)$GENEID[!is.na(mcols(seCombined)$GENEID)]
...
filterTranscripts <- function(seCombined, annotationGrangesList, 
  ...
  if(remove.subsetTx) {
    exonRangesCombinedFiltered <- 
      exonRangesCombinedFiltered[!grepl('compatible', 
  ...
  exonRangesCombinedFiltered <- 
    exonRangesCombinedFiltered[mcols(seCombinedFiltered
                                     )$readClassType != 'equal']
  seCombinedFiltered <- 
  ...
  mcols(seCombinedFiltered)$newTxClass[mcols(
    seCombinedFiltered)$readClassType == 'unsplicedNew' & grepl('gene', 
    mcols(seCombinedFiltered)$GENEID)] <- 'newGene-unspliced'
  mcols(seCombinedFiltered)$newTxClass[mcols(
    seCombinedFiltered)$readClassType=='allNew' & grepl('gene', 
    mcols(seCombinedFiltered)$GENEID)] <- 'newGene-spliced'
  ...
  if(length(extendedAnnotationRanges) > 0){
    mcols(extendedAnnotationRanges)$TXNAME <- paste0('tx', 
  ...
  mcols(annotationRangesToMerge)$newTxClass <- rep('annotation', 
                                                   length(annotationRangesToMerge))
  extendedAnnotationRanges<- c(extendedAnnotationRanges, 
  ...
  geneListWithNewTx <- 
    which(mcols(extendedAnnotationRanges)$GENEID %in% mcols(
      extendedAnnotationRanges)$GENEID[which(mcols(
        extendedAnnotationRanges)$newTxClass!='annotation')])
  minEqClasses <- 
  ...
  mcols(extendedAnnotationRanges)$eqClass[geneListWithNewTx] <- 
    minEqClasses$eqClass[match(names(
      extendedAnnotationRanges[geneListWithNewTx]), minEqClasses$queryTxId)]
  mcols(extendedAnnotationRanges) <- 
    mcols(extendedAnnotationRanges)[, c('TXNAME', 'GENEID', 'eqClass', 
                                        'newTxClass')]
...
filterSet1 = FALSE
if(nrow(se)>0){
  filterSet1 <- rowSums(assays(se)$counts >= min.readCount
                        ) >= min.sampleNumber
}
if(sum(filterSet1) > 0){
  if(any(rowData(se)$confidenceType == 'highConfidenceJunctionReads' & 
    ...
    exonsByReadClass <- createExonByReadClass(seFilteredSpliced, 
                                              annotationGrangesList)
    seFilteredSpliced <- extdannotateSplicedReads(exonsByReadClass,
                                                  intronsByReadClass, annotationGrangesList, seFilteredSpliced, 
                                                  min.exonDistance, verbose)
  }
  ## unspliced transcripts
  SEnRng <- extdannotateUnsplicedReads(se, seFilteredSpliced, 
                                       exonsByReadClass, 
                                       annotationGrangesList, 
  ...
  seCombined <- assignGeneIDexonMatch(seCombined, exonRangesCombined, 
                                      annotationGrangesList, 
                                      min.exonOverlap, prefix, verbose)
  ## filter out transcripts
  extendedAnnotationRanges <- filterTranscripts(seCombined, 
