...
modelME = xgb.load("./inst/extdata/read_class_ME.bin")
modelSE = xgb.load("./inst/extdata/read_class_SE.bin")   


saveRDS(modelME, "./inst/extdata/read_class_ME.json")

raw_modelSE <- xgb.save.raw(modelSE)
saveRDS(raw_modelSE, "./inst/extdata/read_class_SE_raw.rds")

# in 1.6.0.1 xgboost
modelME = xgb.load("./inst/extdata/read_class_ME.model")
xgb.save(modelME, "./inst/extdata/read_class_ME.json")
modelSE = xgb.load("./inst/extdata/read_class_SE.model")
xgb.save(modelSE, "./inst/extdata/read_class_SE.json")
# in 3.1.2.1 xgboost
modelME <- xgb.load("./inst/extdata/read_class_ME.json")
xgb.save(modelME, "./inst/extdata/read_class_ME.ubj")
modelSE <- xgb.load("./inst/extdata/read_class_SE.json")
xgb.save(modelSE, "./inst/extdata/read_class_SE.ubj")
...
for (i in seq_along(standardJunctionModels_temp)) {
  tryCatch({
    # Save directly from the booster object
    xgb.save(standardJunctionModels_temp[[i]], paste0("./inst/extdata/model_", names(standardJunctionModels_temp)[i], ".model"))
    cat("Model", i, "saved\n")
  }, error = function(e) {
...
for (i in seq_along(standardJunctionModels_temp)) {
  tryCatch({
    # Save directly from the booster object
	model <- xgb.load(paste0("./inst/extdata/model_", names(standardJunctionModels_temp)[i], ".model"))
    xgb.save(model, paste0("./inst/extdata/model_", names(standardJunctionModels_temp)[i], ".json"))
    cat("Model", i, "saved\n")
  }, error = function(e) {
    cat("Model", i, "failed:", e$message, "\n")
  })
...
for (i in seq_along(standardJunctionModels_temp)) {
  tryCatch({
    # Save directly from the booster object
	model <- xgb.load(paste0("./inst/extdata/model_", names(standardJunctionModels_temp)[i], ".json"))
    xgb.save(model, paste0("./inst/extdata/model_", names(standardJunctionModels_temp)[i], ".ubj"))
    cat("Model", i, "saved\n")
  }, error = function(e) {
    cat("Model", i, "failed:", e$message, "\n")
  })
...
devtools::load_all("bambu_devel_trial/bambu")
system(paste0("aws s3 "))
#annotations2 = readRDS("hg38_sequins_SIRV_ERCCs_longSIRVs_corrected.rds") # give slightly different training model results 
#annotations <- readRDS("Homo_sapiens.GRCh38.91.sorted.annotations.rds") # format too old, error report for using this, downloaded for bambu code in codeocean
annotations <- prepareAnnotations("Homo_sapiens.GRCh38.91.gtf") # downloaded from sg-nex-data
...
rcf <- bambu(reads = sample, annotations = annotations, genome = fa.file, discovery = FALSE, quant = FALSE, verbose = TRUE)
saveRDS(rcf[[1]], "bambu_devel_trial/SGNex_HepG2_directRNA_replicate5_run1_genome.rds")
se <- readRDS("/usr/data/bambu_devel_trial/SGNex_HepG2_directRNA_replicate5_run1_genome.rds")
defaultModelsNew = trainBambu(se)
xgb.save(defaultModelsNew$transcriptModelME, "./inst/extdata/read_class_ME.ubj")
xgb.save(defaultModelsNew$transcriptModelSE, "./inst/extdata/read_class_SE.ubj")


se <- readRDS("/mnt/projects/update_bambu/dec16_2025/bambu_devel_trial/SGNex_HepG2_directRNA_replicate5_run1_genome.rds")
defaultModelsNew = trainBambu(se)



## install early version of xgboost for debugging
rm -rf ~/R/xgb_temp/00LOCK-xgboost
rm -rf ~/R/xgb_temp/xgboost

#cd /tmp
rm -rf xgboost

# Install 0.90.0.2: generate model file 
#wget https://cran.r-project.org/src/contrib/Archive/xgboost/xgboost_0.90.0.2.tar.gz
tar -xzf xgboost_0.90.0.2.tar.gz


# Try 1.6.0.1: convert model to json file 
#wget https://cran.r-project.org/src/contrib/Archive/xgboost/xgboost_1.6.0.1.tar.gz
tar -xzf xgboost_1.6.0.1.tar.gz


cd xgboost

# Apply patches if needed
sed -i 's/error(XGBGetLastError())/error("%s", XGBGetLastError())/g' src/xgboost_R.cc 2>/dev/null
sed -i 's/error(e.what())/error("%s", e.what())/g' src/xgboost_R.cc 2>/dev/null

R CMD INSTALL --library=~/R/xgb_temp .
